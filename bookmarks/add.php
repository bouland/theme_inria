<?php
	...
	// Get the current page's owner
		...
		$content = elgg_view_title(elgg_echo('bookmarks:this'), false);
		
	// If we've been given a bookmark to edit, grab it
		if ($this_guid = get_input('bookmark',0)) {
			$entity = get_entity($this_guid);
			if ($entity->canEdit()) {
				$content .= elgg_view('bookmarks/form',array('entity' => $entity, 'container_guid' => $container));
			}
		} else {
			$content .= elgg_view('bookmarks/form', array('container_guid' => $container));
		}
