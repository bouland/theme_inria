...
<div class="contentWrapper">
 <div id="notification_box" title="Attention">Voulez-vous envoyer une notification aux membres du groupe ?</div>  

<form id="page_form" action="<?php echo $vars['url']; ?>action/pages/edit" method="post">
	<input type="hidden" name="send_notification" id="send_notification" value="" />
<?php
...
?>
	<p>
		...
		<input type="button" class="submit_button" value="<?php echo elgg_echo("save"); ?>" onClick="notification_box()"/>
	</p>
</form>
	...
	$(document).ready(function() {
		$('#page_form').submit(function() {
	 	...
	 	$( "#notification_box" ).dialog({
	 		autoOpen: false,
		 	modal: true,
		 	buttons: { "Oui": function() {	$( "#send_notification" ).val(true);
		 								   	$(this).dialog("close");
	 										$('#page_form').submit();},
					   "Non": function() { 	$( "#send_notification" ).val(false);
					   					   	$(this).dialog("close");
					   						$('#page_form').submit();}
			         }
		});
